...
if __name__ == '__main__':
	...
	with ProcessPoolExecutor(N) as pool:
		for i,j in itertools.combinations(range(len(layers)), 2):
			futures = [pool.submit(pair_multiplexity, arr.shape, i, j, stops[k], stops[k+1]) for k in range(len(stops) - 1)]
	...
	scaled = [w*w*100 for w in weights]
	...
	nx.draw_networkx_edges(G,pos, width=scaled, edge_color='#6189e1')	#, connectionstyle='Angle, angleA=90, angleB=20, rad=0.2'  , edge_color='#4169e1' '#707070'
